...
import { Alignment, BackgroundView, Box, Button, Dialog, Direction, EqualGrid, IBackgroundConfig, IMediaProps, ITheme, KibaIcon, LinkBase, Markdown, MarkdownText, Media, PaddingSize, ResponsiveContainingView, ResponsiveField, ResponsiveTextAlignmentView, Spacing, Stack, TextAlignment, useTheme } from '@kibalabs/ui-react';
        ...
        <Stack direction={Direction.Vertical} shouldAddGutters={true}>
          ...
          {item.galleryItems && item.galleryItems.length > 0 && (
            <EqualGrid childSizeResponsive={{base: 6}} shouldAddGutters={true}>
              {item.galleryItems.map((galleryItem: IPortfolioGrid1ItemGalleryItem, index: number): React.ReactElement => (
                <LinkBase key={index} target={galleryItem.media.source}>
